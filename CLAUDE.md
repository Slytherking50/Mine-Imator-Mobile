...
| B36 | `[GATE — LEGAL §9.4]` `Data/Minecraft/Game Base.zip` (el único archivo de `Data/Minecraft/` que CMake deja pasar al APK de Android, `CMakeLists.txt:406`) fue silenciosamente reemplazado por el paquete real de texturas de Minecraft 1.20.2 (6.186/6.188 archivos idénticos byte a byte, confirmado por hash) entre el 2026-09-11 y el 2026-09-12 — sin commit ni entrada de sesión que lo documente | **CRÍTICA — CONFIRMADO, sin resolver, requiere decisión del usuario** (encontrado y confirmado 2026-09-16, investigación autónoma) | Timestamps de filesystem + diff de contenido completo + verificación directa contra los 3 `.apk` publicados (148/148/148 coincidencias de nombres reales de Minecraft en cada uno), ver `KNOWN_ISSUES.md` B36 | Las releases públicas ya publicadas `v0.0.2`/`v0.0.3`/`v0.0.4` (`github.com/Slytherking50/Mine-Imator-Mobile`), descargables ahora mismo, **confirmado que distribuyen** el paquete real de Minecraft bajo el nombre "Game Base" — exactamente lo que B23 evaluó y rechazó | **Ninguna acción de remediación tomada todavía** (es una decisión [GATE], no técnica) — recomendación completa (despublicar/reemplazar las releases, restaurar el placeholder legítimo, republicar) en `KNOWN_ISSUES.md` B36 |
...
| 2026-09-16 | §8 (nuevo B36), §9.4 | §9.4 se trataba como un principio firme y respetado sin excepciones desde B23 (2026-09-10) — "no empaquetes texturas... de Minecraft en el APK", con `Game Base.zip` como el placeholder autogenerado que hacía cumplible esa regla | **Violado en la práctica, sin que ninguna sesión lo notara ni lo documentara.** Investigando de forma autónoma (usuario AFK, instrucción explícita de avanzar en pendientes que no lo requirieran) el pendiente legal de "Game Base" que había quedado señalado pero pospuesto a mitad de sesión, se encontró que `Game Base.zip` — el único archivo de `Data/Minecraft/` que `CMakeLists.txt` deja pasar al build de Android — tiene su contenido reescrito el 2026-09-12 21:15 con el paquete real de texturas de Minecraft 1.20.2 (6.186/6.188 archivos idénticos byte a byte contra `1.20.2.zip`, confirmado por hash), probablemente a partir de un archivo de prueba suelto encontrado en la misma carpeta (`Reparado.zip`, 2026-09-11 12:13, contenido idéntico). Sin ningún commit ni entrada de `CLAUDE.md`/`STATUS.md` para el 2026-09-12 que lo explique. Las 3 releases públicas construidas después de esa fecha (`v0.0.2`-`v0.0.4`, todas de esta sesión, 2026-09-15/16) muy probablemente distribuyen ese contenido ahora mismo. **No se tomó ninguna acción de remediación** — es un `[GATE]` legal, decisión del usuario, no de un agente | `KNOWN_ISSUES.md` B36 (evidencia completa, timeline y recomendación) |

| 2026-09-16 | §6.4, §8 (nuevo B38), Fase 4 (pendiente cerrado) | El audit de atajos de teclado sin equivalente táctil había quedado pendiente desde el cierre de Fase 4 (2026-09-15) — "61 call sites, tarea aparte, no gestual" — nunca ejecutado | **Cerrado.** Auditados los ~78 sitios reales (`keyboard_check`/`keyboard_check_pressed`) leyendo cada uno, no solo contándolos. La mayoría son pérdidas seguras: F1-F4/F7/F9/F10 son código muerto (`dev_mode`, falso en release, en cualquier plataforma); Ctrl+X/C/A de textboxes **no es un hueco real** - ya tienen Cortar/Copiar/Pegar/Seleccionar todo en el menú de long-press (`list_init_context_menu.gml:89-101`, mismo mecanismo touch-adaptado desde 2026-09-11); F11/F12, navegación de menú por flechas, y los modificadores Shift sueltos (gizmos, colocar objeto, scroll de menú) son pérdidas menores ya aceptables o dependen de gestos (`mouse_right_pressed`/`mouse_wheel`) que ya están perdidos de por sí en touch. **Un hallazgo real, sin resolver:** la multi-selección (objetos de timeline Y keyframes) depende enteramente de `keyboard_check(vk_shift/vk_control)` en 15+ sitios (`action_tl_select.gml`, `action_tl_select_keyframes*.gml` ×6, `view_click.gml`, `tab_timeline.gml` ×8) - sin teclado físico, cada tap reemplaza la selección entera, imposible operar sobre varios objetos/keyframes a la vez. No implementado a propósito: hay más de un patrón táctil razonable (modo selección persistente, long-press, checkboxes) y ninguno se evaluó con el usuario - `[GATE G4]`, ambigüedad genuina, no mecánico | `KNOWN_ISSUES.md` B38 (detalle completo, sitio por sitio) |
