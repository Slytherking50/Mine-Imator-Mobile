...
| B36 | `[GATE — LEGAL §9.4]` `Data/Minecraft/Game Base.zip` (el único archivo de `Data/Minecraft/` que CMake deja pasar al APK de Android, `CMakeLists.txt:406`) fue silenciosamente reemplazado por el paquete real de texturas de Minecraft 1.20.2 (6.186/6.188 archivos idénticos byte a byte, confirmado por hash) entre el 2026-09-11 y el 2026-09-12 — sin commit ni entrada de sesión que lo documente | **Fuente local REPARADA 2026-09-16 — releases viejas (v0.0.2-v0.0.4) todavía sin remediar** | Verificado 0/0/0 coincidencias de nombres reales de Minecraft en el `.so` del APK reconstruido (antes 148), ver `KNOWN_ISSUES.md` B36 | `Game Base.zip` restaurado desde un artefacto de build limpio ya verificado; `Game Base.midata` reconstruido desde cero (no había copia de respaldo) leyendo el código real para determinar el esquema mínimo necesario, sin copiar ningún valor del `1.20.2.midata` real | Las 3 releases públicas ya publicadas antes de este fix (`v0.0.2`-`v0.0.4`) siguen intactas, sin tocar — **siguen distribuyendo el contenido real**, sigue pendiente la decisión del usuario sobre despublicarlas/reemplazarlas |
